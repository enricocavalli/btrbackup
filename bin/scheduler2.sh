...
while [ 1 ]; do
	...
	#hosts=$(cat $INSTALLDIR/etc/scheduler.conf | grep -v '#' | grep $now | awk {'print $1'} | tr \\n ' ')
	hosts=$(cat $INSTALLDIR/etc/scheduler.conf | grep -v '#' | grep '18:00' | awk {'print $1'} | tr \\n ' ')
	...
	if [ "$hosts" == "" ]; then
		...
		for i in $hosts; do
			...
			#pool=$(cat $INSTALLDIR/etc/scheduler.conf | grep -v '#' | grep $now |grep $i| awk {'print $3'})
			pool=$(cat $INSTALLDIR/etc/scheduler.conf | grep -v '#' | grep '18:00'|grep $i| awk {'print $3'})
